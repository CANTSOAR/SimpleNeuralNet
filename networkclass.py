...
class Model:
    ...
    def train(self, X, y, *, epochs, printevery, validation_data=None, batch_size=None):
        ...
        for epoch in range(1, epochs+1):
            ...
            for step in range(train_steps):
                ...
                if not step % printevery or step == train_steps - 1:
                    print("Step:", step, "Loss:", loss, "(Data Loss:", data_loss, "Reg Loss:", regularization_loss, ")", "Acc:", accuracy, "LR:", self.optimizer.current_learning_rate)
            ...
            print("Epoch Loss:", epoch_loss, "(Epoch Data Loss:", epoch_data_loss, "Epoch Reg Loss:", epoch_reg_loss, ") Acc:", epoch_accuracy, "LR:", self.optimizer.current_learning_rate)
    ...
    def predict(self, X, *, batch_size = None):
        ...
        for step in range(prediction_steps):
            ...
            batch_output = self.forward(X, training = False)
...
def load_custom_data(path):
    ...
    for file in os.listdir(os.path.join(path, "custom")):
        image = cv2.imread(os.path.join(path, "custom", file), cv2.IMREAD_GRAYSCALE)
        image = cv2.resize(image, (28, 28))

        if np.bincount((image[:3] + image[-3:]).reshape(-1)).argmax() >= 200:
            image = 255 - image
...
X = (X.astype("float32") - 127.5) / 125.5
X_test = (X_test.astype("float32") - 127.5) / 125.5
...
model1 = Model.load("fashion_mnist.model")

#model1.evaluate(X_test, y_test, batch_size = 128)

X_custom = load_custom_data("fashion_mnist_images")

X_custom = (X_custom.astype("float32") - 127.5) / 127.5

confidences = model1.predict(X_custom)
...
for prediction, image in zip(predictions, X_custom):
    image = image.reshape(28, 28) * 127.5 + 127.5
